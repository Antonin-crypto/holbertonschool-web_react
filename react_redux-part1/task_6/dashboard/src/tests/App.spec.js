...
describe('Test HOC log mount and unmount "Login" and "CourseList" components', () => {
  ...
  test('Logs when CourseList is mounted and unmounted based on "isLoggedIn" prop value, and handles nameless components', async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        within(logoutSection).getByText("email@example.com")
      ).toBeInTheDocument();
      expect(within(logoutSection).getByText(/logout/i)).toBeInTheDocument();
    });
    expect(console.log).toHaveBeenCalledWith(
      expect.stringMatching(
        /Component CourseList is mounted|Component Component is mounted/
      )
    );
    rerender();
    expect(console.log).toHaveBeenCalledWith(
    ...
    unmount();
    expect(console.log).toHaveBeenCalledWith(
      expect.stringMatching(/Component CourseList is going to unmount/)
    );
    const logCalls = console.log.mock.calls;
    expect(
      logCalls.filter(
        (call) =>
          call[0].includes("Component CourseList is mounted") ||
          call[0].includes("Component Component is mounted")
      ).length
    ).toBeGreaterThanOrEqual(1);
    expect(
      logCalls.filter(
        (call) =>
          call[0].includes("Component CourseList is going to unmount") ||
          call[0].includes("Component Component is going to unmount")
      ).length
    ).toBe(1);
  });
  ...
  test('Logs when Login is mounted and unmounted based on "isLoggedIn" prop value, and handles nameless components', () => {
    ...
    expect(console.log).toHaveBeenCalledWith(
      expect.stringMatching(/Component (Login|Component) is mounted/)
    );
    renderWithProvider(<App />);
    expect(console.log).toHaveBeenCalledWith(
      expect.stringMatching(/Component (Login|Component) is mounted/)
    ...
    unmount();
    expect(console.log).toHaveBeenCalledWith(
      expect.stringMatching(/Component (Login|Component) is going to unmount/)
    );
    const logCalls = console.log.mock.calls;
    expect(
      logCalls.filter(
        (call) =>
          call[0].includes("Component Login is mounted") ||
          call[0].includes("Component Component is mounted")
      ).length
    ).toBe(2);
    expect(
      logCalls.filter(
        (call) =>
          call[0].includes("Component Login is going to unmount") ||
          call[0].includes("Component Component is going to unmount")
      ).length
    ).toBe(1);
  });
...
describe("App Component Tests", () => {
  ...
  test.skip("Verify notification item deletion", async () => {
    ...
    mockAxios.get.mockImplementationOnce(() =>
      Promise.resolve(mockNotificationsResponse)
    );
    renderWithProvider(<App />);
    await waitFor(() => {
    ...
    const notificationCalls = mockAxios.get.mock.calls.filter((call) =>
...
describe("App Component Performance with useCallback", () => {
  ...
  test.skip("HandleDisplayDrawer should maintain referential equality", async () => {
  ...
  test.skip("HandleHideDrawer should maintain referential equality", async () => {
    const { rerender } = renderWithProvider(<App />);
    await waitFor(() => {
      expect(screen.getByText("School Dashboard")).toBeInTheDocument();
    });
  ...
  test.skip("MarkNotificationAsRead should maintain referential equality", async () => {
  ...
  test("Notifications drawer toggle should work via Redux state", async () => {
    const user = userEvent.setup();
    mockAxios.get.mockResolvedValueOnce(mockNotificationsResponse);
